...
# ============================================================================
# PATCH /api/inventory-items/<inventory_item_id>/quantity - Set shelfCount
# ============================================================================

@bp.route('/api/inventory-items/<path:inventory_item_id>/quantity', methods=['PATCH'])
def update_inventory_item_quantity(inventory_item_id):
    """
    API endpoint to set an explicit shelfCount value in an existing InventoryItem.
    Body: {newShelfCount}
    """
    try:
        ...
        if 'newShelfCount' not in data:
            return {'error': 'newShelfCount is required'}, 400

        new_shelf_count = _safe_number(data.get('newShelfCount'), -1)
        if new_shelf_count < 0:
            return {'error': 'newShelfCount must be greater than or equal to 0'}, 400

        target_item = orion.get_entity(inventory_item_id)
        if not target_item:
            return {'error': 'Inventory item not found'}, 404

        shelf_id = target_item.get('refShelf', {}).get('value')
        store_id = target_item.get('refStore', {}).get('value')
        product_id = target_item.get('refProduct', {}).get('value')

        if not shelf_id or not store_id or not product_id:
            return {'error': 'Inventory item has invalid relationships'}, 400
        ...
        shelf_items = orion.get_entities(entity_type='InventoryItem', query=f"refShelf=='{shelf_id}'")
        current_shelf_units = sum(
            _safe_number(item.get('shelfCount', {}).get('value'), 0)
            for item in shelf_items
        )
        current_item_units = _safe_number(target_item.get('shelfCount', {}).get('value'), 0)
        projected_shelf_units = current_shelf_units - current_item_units + new_shelf_count

        if projected_shelf_units > effective_capacity:
            return {
                'error': 'Shelf capacity exceeded',
                'errorCode': 'SHELF_CAPACITY_EXCEEDED',
                'details': {
                    'current': current_shelf_units,
                    'requested': new_shelf_count,
                    'capacity': effective_capacity,
                    'maximumAllowed': max(0, effective_capacity - (current_shelf_units - current_item_units))
                }
            }, 400

        store_items = orion.get_entities(entity_type='InventoryItem', query=f"refStore=='{store_id}'")
        same_product_items = [
            item for item in store_items
            if item.get('refProduct', {}).get('value') == product_id
        ]

        if not same_product_items:
            return {'error': 'No inventory items found for product in store'}, 404

        total_store_stock = 0
        for item in same_product_items:
            item_id = item.get('id')
            if item_id == inventory_item_id:
                total_store_stock += new_shelf_count
            else:
                total_store_stock += _safe_number(item.get('shelfCount', {}).get('value'), 0)

        attrs = {
            'shelfCount': orion.build_attr(new_shelf_count, 'Number'),
            'stockCount': orion.build_attr(total_store_stock, 'Number')
        }
        if not orion.update_entity_attributes(inventory_item_id, attrs):
            return {'error': f'Could not update inventory item {inventory_item_id} in Orion'}, 400

        for item in same_product_items:
            item_id = item.get('id')
            if not item_id or item_id == inventory_item_id:
                continue
            sibling_attrs = {
                'stockCount': orion.build_attr(total_store_stock, 'Number')
            }
            if not orion.update_entity_attributes(item_id, sibling_attrs):
                logger.warning('Could not sync stockCount for InventoryItem %s', item_id)

        return {
            'success': True,
            'inventoryItemId': inventory_item_id,
            'newShelfCount': new_shelf_count,
            'newStockCount': total_store_stock
        }, 200
    except Exception as e:
        logger.error(f"Error updating quantity for inventory item {inventory_item_id}: {e}")
        return {'error': str(e)}, 500
